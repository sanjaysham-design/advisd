...
export default function AdvisorHome({ onSelectClient }) {
  const ytdBarData = CLIENT_DATA.map(c => ({ name: c.name.split(' ')[0], ytd: c.ytd, alpha: c.alpha }))
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 20 }}>
        ...
        <Card title="YTD Return by Client" sub="vs S&P 500 (+8.2%)">
          ...
          <div style={{ fontSize: 9, color: 'var(--tx3)', marginTop: 4 }}>
            <span style={{ display: 'inline-block', width: 16, height: 1, background: '#22c55e', verticalAlign: 'middle', marginRight: 4 }} />
            Blue bars = outperforming S&P 500
          </div>
        </Card>
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
          <thead>
            <tr style={{ borderBottom: '1px solid var(--bdr)' }}>
              {['Client', 'AUM', 'YTD Return', 'Alpha', 'Calls Due', 'Unfunded', 'Liquidity', 'Drift'].map(h => (
            ...
            {CAPITAL_CALLS_CROSS.map((call, i) => (
              <div key={i} style={{
                ...
                <div style={{
                  ...
                  background: urgencyColor[call.urgency],
                }} />
...
function ClientRow({ client: c, idx, onClick }) {
  ...
  return (
    ...
    >
      <td style={{ padding: '9px 10px', fontWeight: 500 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
          ...
          <div>
            ...
            {c.driftFlag && (
              <div style={{ fontSize: 9, color: '#ef4444', fontWeight: 600 }}>⚠ Drift flagged</div>
            )}
          </div>
        </div>
      </td>
      ...
      <td style={{ padding: '9px 10px', textAlign: 'right', fontSize: 12 }}>
        <span style={{ color: c.drift >= 5 ? '#ef4444' : c.drift >= 3 ? '#f59e0b' : 'var(--tx3)' }}>
          {c.drift}%
        </span>
      </td>
    </tr>
...
function KPICard({ label, value, delta, deltaDir, sub, color }) {
  return (
    <div style={{
      background: 'var(--surf)', border: '1px solid var(--bdr)',
